...
{   
    ...
    public function getItemsContaining($value, $category)
    {
        ...
        foreach($categories as $theCat) {
            if((strtolower($theCat->category) == strtolower($value)) || (strtolower($theCat->category) == strtolower($value) . "s")) {
                $catId = $theCat->id;
            }
        }
    ...
    {
        $sql = "SELECT password AS pass FROM user WHERE username = '$username'";
        $query = $this->db->prepare($sql);
        $query->execute();
